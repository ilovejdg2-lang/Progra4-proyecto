...
export default function HistorialComprasCliente() {
    ...
    return (
      <PublicPageGate showLoading={showLoading}>
        <main className="mx-auto max-w-3xl px-4 py-10">
          <p className="text-sm text-slate-600"><ST>Iniciá sesión para ver tu historial de compras.</ST></p>
          <Link to="/login" className="mt-4 inline-flex rounded-full bg-slate-950 px-4 py-2 text-sm font-semibold text-white"><ST>Ir a login</ST></Link>
        </main>
      </PublicPageGate>
    );
  }
  ...
  if (!puedeVer) {
    return (
      <PublicPageGate showLoading={showLoading}>
        <main className="mx-auto max-w-3xl px-4 py-10">
          <p className="text-sm text-slate-600"><ST>Tu rol no tiene acceso al historial de compras.</ST></p>
        </main>
      </PublicPageGate>
    );
  }
      ...
      <main className="mx-auto max-w-4xl space-y-5 px-4 py-8">
        ...
        <section className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <label className="grid gap-1 text-sm">
            ...
            <NumericInput decimal className="min-h-11 rounded-full border border-slate-200 px-3" value={filtros.montoMin} onChange={(e) => { setPage(1); setFiltros((c) => ({ ...c, montoMin: e.target.value })); }} />
          </label>
          <label className="grid gap-1 text-sm">
            <span className="font-medium text-slate-700"><ST>Monto máximo</ST></span>
            <NumericInput decimal className="min-h-11 rounded-full border border-slate-200 px-3" value={filtros.montoMax} onChange={(e) => { setPage(1); setFiltros((c) => ({ ...c, montoMax: e.target.value })); }} />
          </label>
        ...
        {status === "loading" ? (
          <p className="rounded-2xl border border-slate-200 bg-white px-4 py-10 text-center text-sm text-slate-500"><ST>Cargando historial...</ST></p>
        ) : status === "error" ? (
          <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-8 text-center">
            <p className="text-sm font-semibold text-red-700">{error}</p>
            <button type="button" onClick={load} className="mt-3 rounded-full border border-red-700 px-4 py-2 text-sm font-semibold text-red-700"><ST>Reintentar</ST></button>
          </div>
        ) : compras.length === 0 ? (
          <p className="rounded-2xl border border-slate-200 bg-white px-4 py-10 text-center text-sm text-slate-500"><ST>Todavía no tenés compras registradas.</ST></p>
        ) : (
              ...
              <article key={compra.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">{formatFecha(compra.fecha) || tSinFecha}</p>
                    <p className="mt-1 text-sm text-slate-600">{compra.cantidadProductos} <ST>productos</ST> · <ST>{compra.estado}</ST></p>
                  </div>
        ...
        {totalPages > 1 ? (
          <div className="flex items-center justify-end gap-2">
            <button type="button" disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="rounded-full border border-slate-300 px-3 py-1.5 text-sm disabled:opacity-50"><ST>Anterior</ST></button>
            <span className="text-sm text-slate-600">{page} / {totalPages}</span>
            <button type="button" disabled={page >= totalPages} onClick={() => setPage((p) => p + 1)} className="rounded-full border border-slate-300 px-3 py-1.5 text-sm disabled:opacity-50"><ST>Siguiente</ST></button>
          </div>
        ) : null}
        ...
        {detalle ? (
          <AdminModal open onClose={() => setDetalle(null)} maxWidth="max-w-xl" labelledBy="compra-cliente-detalle">
            <AdminModalHeader>
              <h2 id="compra-cliente-detalle" className="text-lg font-semibold"><ST>Compra</ST> {detalle.numero}</h2>
              <button type="button" onClick={() => setDetalle(null)} aria-label={t("Cerrar")} className="rounded-full p-1.5 text-slate-400 hover:bg-slate-100"><X className="size-5" /></button>
            </AdminModalHeader>
            <AdminModalBody>
