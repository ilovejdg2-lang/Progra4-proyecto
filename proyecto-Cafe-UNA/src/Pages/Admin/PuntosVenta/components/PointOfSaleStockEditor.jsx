    ...
    <AdminModal open onClose={onClose} maxWidth="max-w-xl" labelledBy="pos-stock-editor-title">
      ...
      <AdminModalBody>
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          ...
          <label className="grid gap-2 text-sm font-medium text-slate-700">Unidades disponibles<input name="posStock" type="text" inputMode="numeric" value={stock} onChange={(event) => { setStock(event.target.value); setValidationError(""); }} className={`min-h-11 rounded-lg border bg-white px-3 py-2.5 text-base text-slate-900 outline-none transition focus:border-slate-700 focus:ring-2 focus:ring-slate-200 ${message ? "border-red-500" : "border-slate-300"}`} aria-invalid={Boolean(message)} aria-describedby={message ? "pos-stock-error" : "pos-stock-help"} required /><span id="pos-stock-help" className="text-xs font-normal text-slate-500">Usa un número entero entre 0 y {MAX_STOCK}.</span></label>
          <label className="grid gap-2 text-sm font-medium text-slate-700">Motivo del ajuste<textarea name="reason" maxLength={300} rows={3} value={reason} onChange={(event) => { setReason(event.target.value); setValidationError(""); }} className={`rounded-lg border bg-white px-3 py-2.5 text-base text-slate-900 outline-none transition focus:border-slate-700 focus:ring-2 focus:ring-slate-200 ${message ? "border-red-500" : "border-slate-300"}`} aria-invalid={Boolean(message)} aria-describedby={message ? "pos-stock-error" : undefined} required /><span className="text-xs font-normal text-slate-500">{reason.length}/300 caracteres.</span></label>
