/** Divisiones administrativas de Costa Rica para selects en cascada. */
const DIVISIONES = {
  "San José": {
    ...
    Vázquez: ["San Isidro", "San Rafael", "Dulce Nombre de Jesús", "Patalillo", "Cascajal"],
    Acosta: ["San Ignacio", "Guaitil", "Palmichal", "Cangrejal", "Sabanillas"],
    Tibás: ["San Juan", "Cinco Esquinas", "Anselmo Llorente", "León XIII", "Colima"],
    Moravia: ["San Vicente", "San Jerónimo", "Trinidad"],
    Montes: ["San Pedro", "Sabanilla", "Mercedes", "San Rafael"],
    ...
    Pérez: ["San Isidro de El General", "El General", "Daniel Flores", "Rivas", "San Pedro", "Platanares", "Pejibaye", "Cajón", "Barú", "Río Nuevo", "Páramo", "La Amistad"],
  ...
  Puntarenas: {
    ...
    Buenos: ["Buenos Aires", "Volcán", "Potrero Grande", "Boruca", "Pilas", "Colinas", "Chánguena", "Biolley", "Brunka"],
    ...
    Coto: ["San Vito", "Sabalito", "Aguabuena", "Limoncito", "Pittier", "Gutiérrez Braun"],
    Parrita: ["Parrita"],
...
const ALIAS_CANTON = {
  "Vázquez de Coronado": "Vázquez",
  "Montes de Oca": "Montes",
  "Pérez Zeledón": "Pérez",
  "León Cortés Castro": "León Cortés",
  "Buenos Aires": "Buenos",
  "Montes de Oro": "MontesdeOro",
  "Coto Brus": "Coto",
};

export const PROVINCIAS_CR = Object.keys(DIVISIONES);

export function cantonesDeProvincia(provincia) {
  const grupo = DIVISIONES[provincia];
  if (!grupo) return [];
  return Object.keys(grupo).map((clave) => {
    if (clave === "Vázquez") return "Vázquez de Coronado";
    if (clave === "Montes") return "Montes de Oca";
    if (clave === "Pérez") return "Pérez Zeledón";
    if (clave === "León Cortés") return "León Cortés Castro";
    if (clave === "Buenos") return "Buenos Aires";
    if (clave === "MontesdeOro") return "Montes de Oro";
    if (clave === "Coto") return "Coto Brus";
    return clave;
  });
}

function claveCanton(canton) {
  return ALIAS_CANTON[canton] || canton;
}

export function distritosDeCanton(provincia, canton) {
  const grupo = DIVISIONES[provincia];
  if (!grupo) return [];
  return grupo[claveCanton(canton)] || [];
}
